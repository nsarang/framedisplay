(()=>{var x=(n,u)=>()=>(n&&(u=n(n=0)),u);var V=(n,u)=>()=>(u||n((u={exports:{}}).exports,u),u.exports);var w,p=x(()=>{w=`:root {
  --fd-bg-header: #f2f2f2;
  --fd-bg-odd: #fff;
  --fd-bg-even: #f5f5f5;
  --fd-bg-index: #f8f8f8;
  --fd-color-text: rgb(17, 23, 28);
  --fd-color-border: #D1D9E1;
  --fd-color-null: red;
  --fd-bg-null: rgba(68, 83, 95, 0.1);
  --fd-border-null: rgb(232, 236, 241);

  --fd-padding-cell: 0.5em 1.0em 0.5em 0.5em;
  --fd-padding-header: 0.75em 1.5em 0.75em 2.5em;

  --fd-cell-min-width: 2em;
  --fd-cell-max-width: 25em;
  --fd-container-max-height: 500px;
  --fd-font-size: 12px;
  --fd-line-height: 16px;
  --fd-border-width: 0.5px;

  --fd-resizer-width: 8px;
  --fd-resizer-hover-color: rgba(0,0,0,0.1);

  --fd-color-header-text: rgb(17, 23, 28);
  --fd-color-hover: #f0f4f8;
  --fd-color-icon: currentColor;
  --fd-icon-filter: none;
}

.table-container {
    max-height: var(--fd-container-max-height);
    overflow: auto;
}
/* Base table structure */
.frame-display-table {
    width: auto;
    table-layout: auto;
    font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica Neue,Arial,Noto Sans,sans-serif,Apple Color Emoji,Segoe UI Emoji,Segoe UI Symbol,Noto Color Emoji;
    font-size: var(--fd-font-size);
    font-variant-numeric: tabular-nums;
    line-height: var(--fd-line-height);
    color: var(--fd-color-text);
    border-collapse: separate;
    border-spacing: 0;
    border: none;
    /* margin: 0; */
    /* border-top: 0 solid black; */
}
/* Cell styling */
.frame-display-table th,
.frame-display-table td {
    min-width: var(--fd-cell-min-width);
    max-width: var(--fd-cell-max-width);
    padding: var(--fd-padding-cell);
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    border: var(--fd-border-width) solid #D1D9E1;
}
/* Null cell styling */
.frame-display-table tbody code.null-cell {
    font-size: 85%;
    background-color: rgba(68, 83, 95, 0.1);
    border: 1px solid var(--fd-border-null);
    border-radius: 3px;
    padding: 2px 4px;
    display: inline-block;
    color: var(--fd-color-null);
}
/* Row striping for better readability */
.frame-display-table tbody tr:nth-child(odd) td,
.frame-display-table tbody tr:nth-child(odd) th {
    background-color: var(--fd-bg-odd);
}
.frame-display-table tbody tr:nth-child(even) td,
.frame-display-table tbody tr:nth-child(even) th {
    background-color: var(--fd-bg-even);
}
/* Hover effect */
.frame-display-table tbody tr:hover td,
.frame-display-table tbody tr:hover th {
  background-color: var(--fd-color-hover);
}

/* Header */
.frame-display-table thead tr th {
    position: sticky;
    top: 0;
    padding: var(--fd-padding-header);
    text-align: left !important;
    background-color: var(--fd-bg-header);
    font-weight: 600;
    z-index: 10;
    cursor: pointer;
    user-select: none;
    border-collapse: separate;
    color: var(--fd-color-header-text);
    box-shadow: inset 0px 1px 0px 0px #D1D9E1;
}

/* Reserve left area for icons */
.frame-display-table thead th::before {
    content: '';
    position: absolute;
    left: 0.5em;
    top: 50%;
    transform: translateY(-50%);
    width: 1.5em;
    height: 1.5em;
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    filter: var(--fd-icon-filter);
}

/* Data type icons */
.frame-display-table thead th[data-dtype="int"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="float"]::before {
    background-image: url('data:image/svg+xml;base64,');
    top: 55%;
}
.frame-display-table thead th[data-dtype="string"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="datetime"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="bool"]::before {
    background-image: url('data:image/svg+xml;base64,');
    width: 1.7em;
    height: 1.7em;
}
.frame-display-table thead th[data-dtype="category"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="object"]::before {
    background-image: url('data:image/svg+xml;base64,');
}

/* Sticky first column */
.frame-display-table tbody th {
    position: sticky;
    left: 0;
    background-color: var(--fd-bg-index);
    z-index: 1;
    text-align: center;
    box-shadow: inset 1px 0px 0px 0px #D1D9E1;
}
/* Corner cell (intersection of sticky header and first column) */
.frame-display-table thead tr th:first-child {
    position: sticky;
    left: 0;
    top: 0;
    z-index: 20;
    box-shadow: inset 1px 1px 0px 0px #D1D9E1;
}
.frame-display-table.resizing {
  cursor: col-resize;
}
.frame-display-table.resizing * {
  user-select: none !important;
}

/* Sort arrows */
.frame-display-table thead th.sort-asc::after {
    content: ' \u2193';
    font-size: 1em;
    opacity: 0.7;
}

.frame-display-table thead th.sort-desc::after {
    content: ' \u2191';
    font-size: 1em;
    opacity: 0.7;
}

/* Resizer handle */
.fd-column-resizer {
  position: absolute;
  top: 0;
  right: 0;
  width: var(--fd-resizer-width);
  height: 100%;
  cursor: col-resize;
  z-index: 20;
}
.fd-column-resizer:hover {
  background-color: var(--fd-resizer-hover-color);
}

/* ============================================
   DARK THEME (GitHub Dark Dimmed inspired)
   ============================================ */
.theme-dark.frame-display-table {
  --fd-bg-header: #22272e;
  --fd-bg-odd: #2d333b;
  --fd-bg-even: #22272e;
  --fd-bg-index: #22272e;
  --fd-color-text: #adbac7;
  --fd-color-header-text: #cdd9e5;
  --fd-color-border: #444c56;
  --fd-color-hover: #373e47;
  --fd-color-null: #f47067;
  --fd-bg-null: rgba(244, 112, 103, 0.15);
  --fd-border-null: #f47067;
  --fd-icon-filter: brightness(0) invert(0.8);
  --fd-resizer-hover-color: rgba(99, 110, 123, 0.3);
}

/* ============================================
   OCEAN THEME (Professional blue-teal)
   ============================================ */
.theme-ocean.frame-display-table {
  --fd-bg-header: #0f4c75;
  --fd-bg-odd: #f0f8ff;
  --fd-bg-even: #e8f4f8;
  --fd-bg-index: #e1f1f5;
  --fd-color-text: #1b262c;
  --fd-color-header-text: #ffffff;
  --fd-color-border: #3282b8;
  --fd-color-hover: #d4ebf2;
  --fd-color-null: #d9534f;
  --fd-bg-null: rgba(217, 83, 79, 0.1);
  --fd-border-null: #d9534f;
  --fd-icon-filter: brightness(0) invert(1);
  --fd-resizer-hover-color: rgba(50, 130, 184, 0.3);
}

.theme-ocean.frame-display-table thead th {
  font-weight: 600;
}

/* ============================================
   SUNSET THEME (Warm orange-rose)
   ============================================ */
.theme-sunset.frame-display-table {
  --fd-bg-header: #d4504c;
  --fd-bg-odd: #fff8f3;
  --fd-bg-even: #ffe8db;
  --fd-bg-index: #ffdcc7;
  --fd-color-text: #2d1b14;
  --fd-color-header-text: #ffffff;
  --fd-color-border: #e8a598;
  --fd-color-hover: #ffd4bc;
  --fd-color-null: #c73e1d;
  --fd-bg-null: rgba(199, 62, 29, 0.1);
  --fd-border-null: #c73e1d;
  --fd-icon-filter: brightness(0) invert(1);
  --fd-resizer-hover-color: rgba(212, 80, 76, 0.3);
}

.theme-sunset.frame-display-table thead th {
  font-weight: 600;
}

/* ============================================
   NEON THEME (Cyberpunk cyan-magenta)
   ============================================ */
.theme-neon.frame-display-table {
  --fd-bg-header: #1a1a2e;
  --fd-bg-odd: #16213e;
  --fd-bg-even: #0f1624;
  --fd-bg-index: #0f1624;
  --fd-color-text: #00fff5;
  --fd-color-header-text: #ff006e;
  --fd-color-border: #00fff5;
  --fd-color-hover: #1f2b3e;
  --fd-color-null: #ff006e;
  --fd-bg-null: rgba(255, 0, 110, 0.2);
  --fd-border-null: #ff006e;
  --fd-icon-filter: brightness(0) saturate(100%) invert(27%) sepia(99%) saturate(7471%) hue-rotate(318deg) brightness(101%) contrast(106%);
  --fd-resizer-hover-color: rgba(0, 255, 245, 0.4);
}

.theme-neon.frame-display-table thead th {
  font-weight: 700;
  text-shadow: 0 0 10px rgba(255, 0, 110, 0.5);
}

.theme-neon.frame-display-table tbody tr:hover td,
.theme-neon.frame-display-table tbody tr:hover th {
  box-shadow: 0 0 15px rgba(0, 255, 245, 0.1);
}

/* ============================================
   MINIMAL THEME
   ============================================ */
.theme-minimal.frame-display-table {
  --fd-bg-header: #fafafa;
  --fd-bg-odd: #ffffff;
  --fd-bg-even: #ffffff;
  --fd-bg-index: #fafafa;
  --fd-color-text: #1a1a1a;
  --fd-color-header-text: #1a1a1a;
  --fd-color-border: transparent;
  --fd-color-hover: #f5f5f5;
  --fd-border-width: 0;
  --fd-resizer-hover-color: rgba(0, 0, 0, 0.08);
}

.theme-minimal.frame-display-table th,
.theme-minimal.frame-display-table td {
  border-bottom: 1px solid #e5e5e5;
}

.theme-minimal.frame-display-table thead th {
  border-bottom: 2px solid #1a1a1a;
  font-weight: 600;
}

.theme-minimal.frame-display-table tbody th {
  font-weight: 500;
  box-shadow: none;
}

.theme-minimal.frame-display-table thead tr th:first-child {
  box-shadow: none;
}

/* ============================================
   CONTRAST THEME (High Accessibility)
   ============================================ */
.theme-contrast.frame-display-table {
  --fd-bg-header: #000000;
  --fd-bg-odd: #000000;
  --fd-bg-even: #000000;
  --fd-bg-index: #000000;
  --fd-color-text: #ffffff;
  --fd-color-header-text: #ffff00;
  --fd-color-border: #ffffff;
  --fd-color-hover: #333333;
  --fd-border-width: 2px;
  --fd-color-null: #ffff00;
  --fd-bg-null: rgba(255, 255, 0, 0.2);
  --fd-border-null: #ffff00;
  --fd-icon-filter: brightness(0) saturate(100%) invert(100%) sepia(100%) saturate(10000%) hue-rotate(0deg);
  --fd-resizer-hover-color: rgba(255, 255, 0, 0.5);
}

.theme-contrast.frame-display-table {
  border: 3px solid #ffffff;
}

.theme-contrast.frame-display-table thead th {
  font-weight: bold;
  font-size: 1.05em;
}

.theme-contrast.frame-display-table thead th.sort-asc::after,
.theme-contrast.frame-display-table thead th.sort-desc::after {
  color: #ffff00;
}
`});var I,E=x(()=>{I="1.1.6"});var W=V(z=>{p();E();(function(n){"use strict";let u={customCSS:"",theme:"",tableSelector:".frame-display-table",minColumnWidth:30,resizerWidth:8,resizerHoverColor:"rgba(0,0,0,0.1)",showHoverEffect:!0,enableResizing:!0,enableSorting:!0,enableTooltips:!0,enableStickyHeader:!0,enableStickyIndex:!0,autoInit:!0,ReInitialize:!1},a={...u};if(j(),n.FrameDisplay){let e=n.FrameDisplay.version||"0.0.0";if(a.ReInitialize===!0||I.localeCompare(e,void 0,{numeric:!0})>0)n.FrameDisplay.destroy();else return}let L=null;function T(){if(document.getElementById("frame-display-styles"))return;let e=document.createElement("style");e.id="frame-display-styles";let t=`
`;a.enableStickyHeader||(t+=`.frame-display-table thead tr th { position: relative; }
`),a.enableStickyIndex||(t+=`.frame-display-table tbody th { position: relative; }
`),(!a.enableStickyHeader||!a.enableStickyIndex)&&(t+=`.frame-display-table thead tr th:first-child { position: relative; }
`),t+=a.customCSS||"",e.textContent=w+t,document.head.appendChild(e)}function C(e){e.querySelectorAll("thead th").forEach((r,M)=>{if(r.querySelector(".column-resizer"))return;let s=document.createElement("div");s.className="fd-column-resizer",r.appendChild(s),s.addEventListener("click",function(o){o.stopPropagation()});let i,l;s.addEventListener("pointerdown",function(o){i=o.clientX,l=parseFloat(window.getComputedStyle(r).width),s.setPointerCapture(o.pointerId),s.addEventListener("pointermove",c),s.addEventListener("pointerup",d,{once:!0}),e.classList.add("resizing"),document.body.style.userSelect="none",o.preventDefault(),o.stopPropagation()});function c(o){let f=Math.max(a.minColumnWidth,l+(o.clientX-i));r.style.setProperty("width",f+"px","important"),r.style.setProperty("min-width",f+"px","important");let g=e.closest(".table-container");if(g){let N=r.getBoundingClientRect(),y=g.getBoundingClientRect(),h=N.right-y.right;h>0&&(g.scrollLeft+=h)}if(r.getAttribute("data-released")!="true"){r.style.setProperty("max-width",0),r.setAttribute("data-released","true");let N=M+1;e.querySelectorAll(`td:nth-child(${N})`).forEach(y=>{y.style.setProperty("max-width",0)})}}function d(o){o.stopPropagation(),o.preventDefault(),s.releasePointerCapture(o.pointerId),s.removeEventListener("pointermove",c),s.removeEventListener("pointerup",d),document.body.style.userSelect="",setTimeout(()=>e.classList.remove("resizing"),0)}})}function v(e){e.querySelectorAll("th, td").forEach(t=>{let r=t.textContent.trim();r&&(t.title=r)})}function U(e){e.querySelectorAll("thead th").forEach((t,r)=>{t.addEventListener("dblclick",M=>{M.preventDefault()}),t.addEventListener("click",()=>{e.classList.contains("resizing")||Q(e,r+1,t)})})}function Q(e,t,r){let M=e.dataset.sort||"1-asc",[s,i]=M.split("-"),l=parseInt(s),c=l===t&&i==="asc"?"desc":l===t&&i==="desc"?"none":"asc";if(l!==t){let f=e.querySelector(`thead th:nth-child(${l})`);f&&S(f)}e.dataset.sort=`${t}-${c}`;let d=e.querySelector("tbody"),o=Array.from(d.querySelectorAll("tr"));c==="none"?D(o,1,"asc","object").forEach(g=>d.appendChild(g)):D(o,t,c,r.dataset.dtype).forEach(g=>d.appendChild(g)),Y(r,c)}function Y(e,t){S(e),t!=="none"&&e.classList.add(`sort-${t}`)}function S(e){e.classList.remove("sort-asc","sort-desc")}function D(e,t,r,M){return e.map(i=>{let l=i.querySelector(`td:nth-child(${t}), th:nth-child(${t})`),c=l?.querySelector(".null-cell"),d=l?.textContent.trim()||"",o=d;if(!c&&d)switch(M){case"int":case"float":o=parseFloat(d)||d;break;case"datetime":o=new Date(d).getTime()||d;break;case"bool":o=/^(true|1|yes|on|t|y)$/i.test(d)?1:0;break;default:t===1&&(o=parseFloat(d)||d)}return{row:i,isNull:c,sortValue:o}}).sort((i,l)=>{if(i.isNull!==l.isNull)return i.isNull?1:-1;let c=typeof i.sortValue=="number"?i.sortValue-l.sortValue:i.sortValue.localeCompare(l.sortValue,void 0,{numeric:!0,sensitivity:"base"});return r==="asc"?c:-c}).map(i=>i.row)}function A(e){e.hasAttribute("data-initialized")||(j(),a.theme&&e.classList.add(`theme-${a.theme}`),a.enableResizing&&C(e),a.enableTooltips&&v(e),a.enableSorting&&U(e),e.setAttribute("data-initialized","true"))}function j(){a={...u,...n.FrameDisplayConfig||{}}}function m(){j(),T(),document.querySelectorAll(a.tableSelector).forEach(A)}function O(){L&&(L.disconnect(),L=null),document.querySelectorAll(".frame-display-table[data-initialized]").forEach(t=>{t.querySelectorAll(".column-resizer").forEach(r=>{r.remove()}),t.querySelectorAll("thead th").forEach(r=>{let M=r.cloneNode(!0);r.parentNode.replaceChild(M,r),M.classList.remove("sort-asc","sort-desc")}),t.dataset.sort&&delete t.dataset.sort,t.removeAttribute("data-initialized"),t.classList.remove("resizing"),t.className=t.className.split(" ").filter(r=>!r.startsWith("theme-")).join(" ")});let e=document.getElementById("frame-display-styles");return e&&e.remove(),n.FrameDisplay&&delete n.FrameDisplay,!0}function k(){if(!(typeof MutationObserver>"u"))return L=new MutationObserver(e=>{e.forEach(t=>{t.type==="childList"&&t.addedNodes.length>0&&t.addedNodes.forEach(r=>{if(r.nodeType===Node.ELEMENT_NODE){let M=[];r.matches&&r.matches(".frame-display-table")&&M.push(r),M.push(...r.querySelectorAll(".frame-display-table:not([data-initialized])")),M.forEach(A)}})})}),L.observe(document.body,{childList:!0,subtree:!0}),L}function b(){a.autoInit!==!1&&(m(),k())}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",b):setTimeout(b,0),n.FrameDisplay={setup:m,destroy:O,version:I,get settings(){return{...a}}}})(typeof window<"u"?window:z)});W();})();
